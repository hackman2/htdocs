...
__d('CavalryLoggerImpl', ['Arbiter', 'Banzai', 'Bootloader', 'ImageTimingHelper', 'ISB', 'KillabyteProfilerConfig', 'NavigationTimingHelper', 'PageEvents', 'PageletEventConstsJS', 'PageletEventsHelper', 'PerfXLogger', 'ResourceTimingBootloaderHelper', 'ScriptPath', 'TimeSliceHelper', 'performance', 'performanceAbsoluteNow', '__getTotalFactories', '__getCompileTime', '__getFactoryTime'], (function a(b, c, d, e, f, g) {
    ...
    Object.assign(l.prototype, {
        initializeInstrumentation: function m() {
            ...
            c('Arbiter').subscribe(c('PageEvents').BIGPIPE_ONLOAD, function(event, o) { if (b.bigPipe && n.lid === b.bigPipe.lid) n.addPiggyback('cjs_factory_count_e2e', c('__getTotalFactories')()).addPiggyback('cjs_compile_time_e2e', c('__getCompileTime')()).addPiggyback('cjs_factory_time_e2e', c('__getFactoryTime')()); });
        ...
        recordLoadedEvent: function m(n) {
            var o = this;
            n.subscribe('all_pagelets_loaded', function(event, p) { if (p.lid === o.lid) o.setAbsTimeStamp('t_marker_bigpipe_e2e', p.ts, true); });
        },
        ...
        setTTIPhase: function m(n) { this.addPiggyback('tti_phase', n); return this; },
        ...
        setAbsTimeStamp: function m(n, o, p, q) { this.setValue(n, o, p, q); if (this.tti_event && n == this.tti_event) this.measurePageLoad(p, o); return this; },
        logE2E: function m() {
            var n = { lid: this.lid, t_creport: c('performanceAbsoluteNow')(), cavalry_e2e: true };
            if (c('ISB').token) n.fb_isb = c('ISB').token;
            for (var o = 0; o < k.length; o++) n[k[o]] = this.values[k[o]];
            c('Banzai').post(j, n, { signal: true, retry: false });
            this.e2eLogged = true;
        },
        log: function m() {
            ...
            var o = babelHelpers['extends']({ t_creport: c('performanceAbsoluteNow')() }, n, this.values, this.piggy_values),
            ...
            if (p.getCodeUsage && i) {
                ...
                var q = i.findUsedCSSSelectors(document, i.getDocumentStylesheets(document)),
                    r = { js_calls: JSON.stringify(p.getCodeUsage()), css_selectors: JSON.stringify(q), bootloads: JSON.stringify(this.getBootloadsUntil(this.e2e)) };
                if (h) r.html = JSON.stringify(h.getKillabyteHTMLData());
                var s = babelHelpers['extends']({}, i.getDataForSnapshot(this.ddUsageSnapshot), { bootloads: JSON.stringify(this.getBootloadsUntil(this.dd)) }),
                    t = { e2e: r, dd: s };
                if (this.ttiUsageSnapshot) {
                    if (!this.tti) throw new Error('tti timestamp should always be present if we have usage data');
                    t.tti = babelHelpers['extends']({}, i.getDataForSnapshot(this.ttiUsageSnapshot), { bootloads: JSON.stringify(this.getBootloadsUntil(this.tti)) });
                }
                c('Arbiter').inform('cavalry_usage_data_collected', { usageData: t, lid: this.lid }, c('Arbiter').BEHAVIOR_STATE);
                p.clearCodeUsage();
            }
        ...
        logPiggybacks: function m() {
            this.moveBootloaderData();
            var n = babelHelpers['extends']({ lid: this.lid }, this.piggy_values);
            this.piggy_values = {};
            c('Banzai').post(j, n, c('Banzai').VITAL);
        },
        moveBootloaderData: function m() { if (this.log_resources) this.addPiggyback('resource_timing_bootloader', c('ResourceTimingBootloaderHelper').mergeBootloaderMetricsAndResourceTimings(this.piggy_values.resource_timing_bootloader, this.bootloader_metrics, false, {})); },
        ...
        measurePageLoad: function m(n, o) {
            if (o) { this.setAbsTimeStamp('t_tti', o, n); } else this.setTimeStamp('t_tti', n);
        ...
        collectTiming: function m(n, o, p) { var q = ['connectEnd', 'connectStart', 'domComplete', 'domContentLoadedEventEnd', 'domContentLoadedEventStart', 'domInteractive', 'domLoading', 'domainLookupEnd', 'domainLookupStart', 'fetchStart', 'loadEventEnd', 'loadEventStart', 'navigationStart', 'redirectEnd', 'redirectStart', 'requestStart', 'responseEnd', 'responseStart', 'secureConnectionStart', 'startTime', 'unloadEventEnd', 'unloadEventStart']; for (var r = 0; r < q.length; r++) { var s = q[r]; if (o[s]) this.setAbsTimeStamp(n + s, o[s] + p, true); } }
    });
    Object.assign(l, {
