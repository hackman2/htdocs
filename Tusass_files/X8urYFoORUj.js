if (self.CavalryLogger) { CavalryLogger.start_js(["\/8vNF"]); }
    ...
    var h = 'browse_client_event_session',
        ...
        k = {
            logData: function l(event, m, n) {
                ...
                c('Banzai').post(h, [o], { delay: 0, retry: true, signal: p });
            },
            ...
            maybeLogVisiblityEvent: function l(m) {
                ...
                if (!m || !m.xt || m.xt.indexOf(j) !== 0) { if (n) c('Banzai').post(i, [o], { delay: 0, retry: true }); return; }
...
__d('PageHooks', ['Arbiter', 'ErrorUtils', 'InitialJSLoader', 'PageEvents'], (function a(b, c, d, e, f, g) {
    var h = { DOMREADY_HOOK: 'domreadyhooks', ONLOAD_HOOK: 'onloadhooks' };
    ...
    function l(p) {
        ...
        do { var r = window[p]; if (!r) break; if (!q) window[p] = null; for (var s = 0; s < r.length; s++) { var t = k(r[s], p); if (q && t) return t; } } while (!q && window[p]);
    }
    ...
    var o = babelHelpers['extends']({ _domreadyHook: i, _onloadHook: j, runHook: k, runHooks: l, keepWindowSetAsLoaded: m }, h);
    n();
...
__d('DOMControl', ['DataStore', '$'], (function a(b, c, d, e, f, g) {
    ...
    h.prototype.getRoot = function() { 'use strict'; return this.root; };
    ...
    h.prototype.onupdate = function(i) { 'use strict'; };
    h.getInstance = function(i) { 'use strict'; return c('DataStore').get(i, 'DOMControl'); };
    f.exports = h;
}), null);
__d('Input', ['CSS', 'DOMQuery', 'DOMControl'], (function a(b, c, d, e, f, g) {
    var h = {
        isWhiteSpaceOnly: function i(j) { return !/\S/.test(j || ''); },
        isEmpty: function i(j) { return h.isWhiteSpaceOnly(j.value); },
        getValue: function i(j) { return h.isEmpty(j) ? '' : j.value; },
        getValueRaw: function i(j) { return j.value; },
        ...
        setSubmitOnEnter: function i(j, k) { c('CSS').conditionClass(j, 'enter_submit', k); },
        getSubmitOnEnter: function i(j) { return c('CSS').hasClass(j, 'enter_submit'); },
        setMaxLength: function i(j, k) { if (k > 0) { j.setAttribute('maxlength', k); } else j.removeAttribute('maxlength'); }
    };
    f.exports = h;
}), null);
...
__d('FlipDirectionOnKeypress', ['Event', 'FlipDirection'], (function a(b, c, d, e, f, g) {
    ...
    c('Event').listen(document.documentElement, { keyup: h, input: h });
}), null);
__d("ScriptPathState", ["Arbiter"], (function a(b, c, d, e, f, g) {
    var h, i, j, k, l = 100,
        m = {
            setIsUIPageletRequest: function n(o) { j = o; },
            setUserURISampleRate: function n(o) { k = o; },
            reset: function n() {
                h = null;
                i = false;
                j = false;
            },
            _shouldUpdateScriptPath: function n() { return i && !j; },
            _shouldSendURI: function n() { return Math.random() < k; },
            getParams: function n() { var o = {}; if (m._shouldUpdateScriptPath()) { if (m._shouldSendURI() && h !== null) o.user_uri = h.substring(0, l); } else o.no_script_path = 1; return o; }
        };
...
__d("goOrReplace", [], (function a(b, c, d, e, f, g) {
    function h(i, j, k) { if (k) { i.replace(j); } else if (i.href == j) { i.reload(); } else i.href = j; }
    f.exports = h;
}), null);
__d('AjaxPipeRequest', ['Arbiter', 'AsyncRequest', 'AsyncRequestConfig', 'BigPipe', 'CSS', 'DOM', 'Env', 'PageEvents', 'PageletSet', 'PageNavigationStageLogger', 'ScriptPathState', 'URI', 'containsNode', 'ge', 'goOrReplace', 'performance', 'performanceAbsoluteNow', 'PageletGK'], (function a(b, c, d, e, f, g) {
    ...
    function k(n, o) {
        ...
        for (var r = 0; r < q.length; r++) { var s = q[r]; if (c('containsNode')(p, c('ge')(s))) c('PageletSet').removePagelet(s); }
        c('Arbiter').inform(c('PageEvents').AJAXPIPE_ONBEFORECLEARCANVAS, { canvasID: n });
        if (h) {
            c('Arbiter').inform('pagelet/destroy', { id: null, root: p });
            c('DOM').empty(p);
        } else {
            c('DOM').empty(p);
            c('Arbiter').inform('pagelet/destroy', { id: null, root: p });
    ...
    function l(n, o) { var p = c('ge')(n); if (p && !o) p.style.minHeight = '100px'; }
    ...
    m.prototype.getData = function(n) { 'use strict'; return this._query_data; };
    ...
    m.prototype.getAsyncRequestType = function() { 'use strict'; if (c('AsyncRequestConfig').useFetchStreamAjaxPipeTransport) return 'useFetchWithIframeFallback'; return 'useIframeTransport'; };
    m.prototype.send = function() {
        'use strict';
        this._arbiter.inform(c('PageEvents').AJAXPIPE_SEND, { rid: this._requestID, quickling: !!this._isQuickling, ts: c('performanceAbsoluteNow')() }, c('Arbiter').BEHAVIOR_PERSISTENT);
        var n = { ajaxpipe: 1, ajaxpipe_token: c('Env').ajaxpipe_token };
        ...
        if (this._automatic) { this._relevantRequest = i; } else i = this._request;
        if (this._isQuickling) { var o = c('performance').clearResourceTimings || c('performance').webkitClearResourceTimings; if (o) o.call(c('performance')); }
        this._request.send();
        return this;
    };
    m.prototype._preBootloadFirstResponse = function(n) { 'use strict'; return false; };
    ...
    m.prototype._fireOnloadCallback = function() {
        ...
        this._arbiter.inform(c('PageEvents').AJAXPIPE_ONLOAD, { lid: this.pipe.lid, rid: this._requestID, ts: c('performanceAbsoluteNow')() }, c('Arbiter').BEHAVIOR_STATE);
    };
    m.prototype._isRelevant = function(n) { 'use strict'; return this._request == i || this._automatic && this._relevantRequest == i || this._jsNonBlock || i && i._allowIrrelevantRequests; };
    m.prototype._preBootloadHandler = function(n) {
        ...
        if (n.is_first) {
            ...
            this.pipe = new(c('BigPipe'))({ __sf: n.__sf, arbiter: this._arbiter, rootNodeID: this._canvas_id, lid: this._request.lid, rid: this._requestID, isAjax: true, domContentCallback: this._fireDomContentCallback.bind(this), onloadCallback: this._fireOnloadCallback.bind(this), domContentEvt: c('PageEvents').AJAXPIPE_DOMREADY, onloadEvt: c('PageEvents').AJAXPIPE_ONLOAD, jsNonBlock: this._jsNonBlock, automatic: this._automatic, displayCallback: this._displayCallback, allowIrrelevantRequests: this._allowIrrelevantRequests });
        }
        return p;
    };
    ...
    m.prototype.isPageActive = function(n) { 'use strict'; return true; };
    m.prototype.getSanitizedParameters = function() { 'use strict'; return []; };
    m.prototype._versionCheck = function(n) { 'use strict'; return true; };
    m.prototype._onInitialResponse = function(n) { 'use strict'; var o = n.getPayload(); if (!this._isRelevant(n)) return false; if (!o) return true; if (this._redirect(o) || !this._versionCheck(o)) return false; return true; };
    m.prototype._processFirstResponse = function(n) { 'use strict'; var o = n.getPayload(); if (c('ge')(this._canvas_id) && o.canvas_class != null) c('CSS').setClass(this._canvas_id, o.canvas_class); };
    ...
    m.prototype._onResponse = function(n) {
        ...
        if (o) {
            ...
            if (o.secondFlushPayload) { this.pipe.setSecondFlushPayload(o.secondFlushPayload); } else this.pipe.onPageletArrive(o);
        }
    ...
    m.prototype.getAsyncRequest = function() { 'use strict'; return this._request; };
    m.getCurrentRequest = function() { 'use strict'; return i; };
...
__d('ContextualThing', ['CSS', 'containsNode', 'ge', 'getOrCreateDOMID'], (function a(b, c, d, e, f, g) {
    var h = {
        register: function i(j, k) { j.setAttribute('data-ownerid', c('getOrCreateDOMID')(k)); },
        ...
        parentByClass: function i(j, k) {
            ...
            while (l && !c('CSS').hasClass(l, k))
                if (l.getAttribute && (m = l.getAttribute('data-ownerid'))) { l = c('ge')(m); } else l = l.parentNode;
            return l;
        }
    };
    f.exports = h;
    ...
    var h = new(c('Map'))(),
        i = {
            ...
            addToQueue: function j(k, l, m, n, o, p, q) {
                if (this.autoFlushCookies) { if (o) { c('Cookie').setWithoutChecksIfFirstPartyContext(k, l, m, n, q); } else c('Cookie').setWithoutChecks(k, l, m, n, q); return; }
                if (h.has(k)) return;
                h.set(k, { name: k, value: l, nMilliSecs: m, path: n, firstPartyOnly: o, secure: q });
                if (p) this.addDefaultInteractionListener();
            },
            flushAllCookies: function j() {
                delete c('Env').defer_cookies;
                h.forEach(function(l, m) { if (l.firstPartyOnly) { c('Cookie').setWithoutChecksIfFirstPartyContext(l.name, l.value, l.nMilliSecs, l.path, l.secure); } else c('Cookie').setWithoutChecks(l.name, l.value, l.nMilliSecs, l.path, l.secure); });
            ...
            getNoticePolicyVersion: function j() { try { var k = document.getElementById('cpn-pv-link'); if (!k) return '1'; return k.getAttribute('data-pv'); } catch (l) { return '1'; } },
            removeDefaultInteractionListener: function j() {
                if (this.defaultHandler) {
                    if (window.removeEventListener) { window.removeEventListener('click', this.defaultHandler, true); } else if (document.detachEvent) document.detachEvent('onclick', this.defaultHandler);
            ...
            addDefaultInteractionListener: function j() {
                if (this.shouldAddDefaultListener) {
                    ...
                    if (window.addEventListener) { window.addEventListener('click', this.defaultHandler, true); } else if (document.attachEvent) document.attachEvent('onclick', this.defaultHandler);
                }
            },
            registerCallbackOnCookieFlush: function j(k) { if (this.autoFlushCookies) { k(); } else this.callbacks.add(k); },
            baseInteractionHandler: function j() { this.flushAllCookies(); },
            canEmbedThirdPartyPixel: function j() { if (c('Env').no_cookies || c('Env').defer_cookies) return false; return this.autoFlushCookies || h.size === 0; }
        };
    f.exports = i;
}), null);
__d('Form', ['DataStore', 'DOM', 'DOMQuery', 'DTSG', 'Input', 'LSD', 'Random', 'PHPQuerySerializer', 'URI', 'getElementPosition', 'isFacebookURI', 'isNode'], (function a(b, c, d, e, f, g) {
    ...
    var k = {
        getInputs: function l() { var m = arguments.length <= 0 || arguments[0] === undefined ? document : arguments[0]; return [].concat(c('DOMQuery').scry(m, 'input'), c('DOMQuery').scry(m, 'select'), c('DOMQuery').scry(m, 'textarea'), c('DOMQuery').scry(m, 'button')); },
        ...
        getSelectValue: function l(m) { return m.options[m.selectedIndex].value; },
        setSelectValue: function l(m, n) {
            for (var o = 0; o < m.options.length; ++o)
                if (m.options[o].value == n) { m.selectedIndex = o; break; }
        },
        ...
        getElements: function l(m) { return m.tagName == 'FORM' && m.elements != m ? Array.from(m.elements) : k.getInputs(m); },
        getAttribute: function l(m, n) { return (m.getAttributeNode(n) || {}).value || null; },
        ...
        forEachValue: function l(m, n, o) {
            k.getElements(m).forEach(function(p) {
                ...
                if (p.nodeName === 'SELECT') { for (var q = 0, r = p.options.length; q < r; q++) { var s = p.options[q]; if (s.selected) o('select', p.name, s.value); } return; }
                if (p.type === 'file') { if (h) { var t = p.files; for (var u = 0; u < t.length; u++) o('file', p.name, t.item(u)); } return; }
        ...
        createFormData: function l(m, n) {
            ...
            var o = new FormData();
            if (m)
                if (c('isNode')(m)) { k.forEachValue(m, n, function(r, s, t) { o.append(s, t); }); } else {
                    var p = j(m);
                    for (var q in p)
                        if (p[q] == null) { o.append(q, ''); } else o.append(q, p[q]);
                }
            return o;
        },
        ...
        _serializeHelper: function l(m, n, o) {
            ...
            if (q) {
                ...
                var s = 0;
                if (q[2] === '') { while (m[q[1]][s] !== undefined) s++; } else s = q[2];
                if (q[3] === '') { m[q[1]][s] = o; } else k._serializeHelper(m[q[1]], s.concat(q[3]), o);
            } else m[n] = o;
        },
        _serializeFix: function l(m) {
            ...
            o.sort(function(r, s) { return r - s; });
            var p = 0,
                q = o.every(function(r) { return +r === p++; });
            if (q) return o.map(function(r) { return m[r]; });
            return m;
        },
        post: function l(m, n, o) {
            ...
            if (o) {
                if (s) { r.rel = 'noopener'; if (o === '_blank') { o = btoa(c('Random').uint32()); var t = window.open('about:blank', o); if (!(t === undefined)) t.opener = null; } }
                r.target = o;
            }
            if (p)
                if (!s && q.getSubdomain() !== 'apps') { n.fb_dtsg = c('DTSG').getToken(); if (c('LSD').token) n.lsd = c('LSD').token; }
        ...
        createHiddenInputs: function l(m, n, o) {
            ...
            for (var r in q) {
                ...
                if (o[r] && p) { o[r].value = q[r]; } else {
                    var s = c('DOM').create('input', { type: 'hidden', name: r, value: q[r] });
                    o[r] = s;
                    n.appendChild(s);
                }
            }
        ...
        getFirstElement: function l(m) {
            var n = arguments.length <= 1 || arguments[1] === undefined ? ['input[type="text"]', 'textarea', 'input[type="password"]', 'input[type="button"]', 'input[type="submit"]'] : arguments[1],
                o = [];
            for (var p = 0; p < n.length; p++) { o = c('DOMQuery').scry(m, n[p]); for (var q = 0; q < o.length; q++) { var r = o[q]; try { var s = c('getElementPosition')(r); if (s.y > 0 && s.x > 0) return r; } catch (t) {} } }
            return null;
        },
        focusFirst: function l(m) { var n = k.getFirstElement(m); if (n) { n.focus(); return true; } return false; }
    };
    f.exports = k;
}), null);
...
__d('OnVisible', ['Arbiter', 'DOM', 'Event', 'Parent', 'Run', 'Vector', 'ViewportBounds', 'coalesce', 'queryThenMutateDOM'], (function a(b, c, d, e, f, g) {
    ...
    function s() {
        for (var w = Math.min(h.length, j) - 1; w >= 0; --w) {
            var x = h[w];
            if (!x.elementPos || x.removed) { h.splice(w, 1); continue; }
    ...
    function u() { c('queryThenMutateDOM')(r, s, 'OnVisible/positionCheck'); }
    ...
    v.prototype.checkBuffer = function() { u(); };
    v.prototype.getElement = function() { return this.element; };
    v.prototype.$OnVisible1 = function() { return c('Event').listen(c('DOM').find(this.scrollArea, '.uiScrollableAreaWrap'), 'scroll', this.checkBuffer); };
    Object.assign(v, { checkBuffer: u });
    f.exports = v;
}), null);
__d('PopoverMenuInterface', ['ArbiterMixin', 'emptyFunction', 'mixin'], (function a(b, c, d, e, f, g) {
    ...
    Object.assign(j.prototype, { getRoot: c('emptyFunction'), onShow: c('emptyFunction'), onHide: c('emptyFunction'), focusAnItem: c('emptyFunction').thatReturnsFalse, blur: c('emptyFunction'), handleKeydown: c('emptyFunction').thatReturnsFalse, destroy: c('emptyFunction') });
    f.exports = j;
}), null);
__d('Toggler', ['csx', 'Arbiter', 'ArbiterMixin', 'ContextualThing', 'CSS', 'DataStore', 'DOM', 'DOMQuery', 'Event', 'Focus', 'Keys', 'Parent', 'TabbableElements', 'TimeSlice', 'arrayContains', 'createArrayFromMixed', 'emptyFunction', 'ge', 'getContextualParent', 'getObjectValues', 'mixin', 'setImmediate', '$'], (function a(b, c, d, e, f, g, h) {
    ...
    function n() {
        if (!m) {
            m = true;
            c('setImmediate')(function() { m = false; });
        }
    }
    var o = function s() {
        o = c('emptyFunction');
    ...
    p.prototype.toggle = function(s) {
        'use strict';
        var t = q(this, s);
        if (t.active === s) { t.hide(); } else t.show(s);
        n();
    };
    p.prototype.getActive = function() { 'use strict'; return q(this).active; };
    p.prototype.isShown = function() { 'use strict'; return q(this).active && c('CSS').hasClass(q(this).active, 'openToggler'); };
    p.isNodeShown = function(s) { 'use strict'; return c('CSS').hasClass(s, 'openToggler'); };
    p.prototype.inTargetFlyout = function(s) { 'use strict'; var t = r(this.getActive()); return t && c('ContextualThing').containsIncludingLayers(t, s); };
    p.prototype.inActiveDialog = function() { 'use strict'; var s = b.Dialog && b.Dialog.getCurrent(); return !s || c('DOM').contains(s.getRoot(), this.getActive()); };
    ...
    p.prototype.setSticky = function(s) {
        ...
        var t = q(this);
        s = s !== false;
        if (s !== t.sticky) { t.sticky = s; if (s) { t.$Toggler1 && t.$Toggler1.unsubscribe(); } else t.$Toggler1 = c('Arbiter').subscribe('pre_page_transition', t.hide.bind(t, null)); }
        return t;
    };
        ...
        return p.subscribe(c('createArrayFromMixed')(s), function(v, w) {
            if (w.getActive() === t) {
                if (w.__continuation) {
                    ...
                    return x(function() { return u(v, w); });
                }
                return u(v, w);
            }
        });
    ...
    Object.assign(p, { subscribe: function(s) { return function(t, u) { t = c('createArrayFromMixed')(t); if (c('arrayContains')(t, 'show')) k.forEach(function(v) { if (v.getActive()) setTimeout(u.bind(null, 'show', v), 0); }); return s(t, u); }; }(p.subscribe.bind(p)) });

    function q(s, t) { if (s instanceof p) return s; return p.getInstance(t); }

    function r(s) { var t = c('DOM').scry(s, 'a[rel="toggle"]'); if (t.length > 0 && t[0].getAttribute('data-target')) return c('ge')(t[0].getAttribute('data-target')); }
    f.exports = p;
}), null);
__d('VirtualCursorStatus', ['Event', 'KeyStatus', 'Set', 'UserAgent', 'emptyFunction', 'setImmediate'], (function a(b, c, d, e, f, g) {
    ...
    function l(event) {
        ...
        if (!h && !i && !r && v && c('UserAgent').isBrowser('IE >= 10') && z == null)
            if (s < 0 && t < 0) { q.add('IE'); } else if ((w < 0 || w > da) && (x < 0 || x > ea)) q.add('MSIE');
    ...
    function m() {
        h = true;
        c('setImmediate')(function() { h = false; });
    }

    function n() {
        i = true;
        c('setImmediate')(function() { i = false; });
    }
    ...
    var p = {
        isVirtualCursorTriggered: function q() { return k; },
        add: function q(r) {
            var s = arguments.length <= 1 || arguments[1] === undefined ? c('emptyFunction') : arguments[1],
                t = c('Event').listen(r, 'click', function(event) { return o(event, s); }),
...
__d('FacebarPlaceholderShortcut', ['KeyEventController', 'Run', 'setTimeoutAcrossTransitions'], (function a(b, c, d, e, f, g) {
    ...
    h.prototype.enable = function() { this._registerListener(); };
    h.prototype._registerListener = function() {
        ...
        c('Run').onLeave(function() { c('setTimeoutAcrossTransitions')(this._registerListener.bind(this), 0); }.bind(this));
    };
    h.prototype.disable = function() {
        this._listener && this._listener.remove();
        this._listener = null;
    };
    h.prototype._handleKeydown = function(i) { this._input.focus(); return false; };
    f.exports = h;
}), null);
...
__d('FacebarBootloader', ['AsyncRequest', 'BanzaiODS', 'CSS', 'Event', 'FacebarPlaceholderShortcut', 'Run', 'SubscriptionsHandler', 'XFacebarBootloadController', 'getActiveElement'], (function a(b, c, d, e, f, g) {
    ...
    var h = 'facebar_bootloader',
        ...
        j = {
            ...
            showLoadingIndicator: function k() { if (this.loadingIndicator) c('CSS').show(this.loadingIndicator); },
            reset: function k() {
            ...
            requestSearch: function k() {
                ...
                m.setURI(l).setMethod('GET').setReadOnly(true).setAllowCrossPageTransition(true).setErrorHandler(function(n) {
                    ...
                    if (n && n.errorSummary) { c('BanzaiODS').bumpEntityKey(h, i + 'request_failed_' + n.errorSummary); } else c('BanzaiODS').bumpEntityKey(h, i + 'request_failed_no_error_summary');
                    this.isRequested = false;
                }).send();
            },
            setFocus: function k(l, m, n) {
                ...
                if (this.searchInput.value || this.isFocused) {
                    ...
                    m.focus();
                    m.setSelection({ length: this.searchInput.selectionEnd - this.searchInput.selectionStart, offset: this.searchInput.selectionStart });
...
__d('FacebarStructuredFragment', [], (function a(b, c, d, e, f, g) {
    ...
    function h(k, l) { if (k && l) { return k.toLowerCase() == l.toLowerCase(); } else return !k && !l; }
    ...
    j.prototype.getText = function() { return this._text; };
    j.prototype.getUID = function() { return this._uid; };
    j.prototype.getType = function() { return this._type; };
    j.prototype.getTypePart = function(k) { return this._getTypeParts()[k]; };
    j.prototype.getLength = function() { return this._text.length; };
    j.prototype.isType = function(k) {
        for (var l = 0; l < arguments.length; l++)
            if (!h(arguments[l], this.getTypePart(l))) return false;
        return true;
    };
    j.prototype.isLocal = function() { return this._isLocal; };
    j.prototype.getCategoryName = function() { return this._categoryName; };
    j.prototype.isWhitespace = function() { return /^\s*$/.test(this._text); };
    j.prototype.hasRTL = function() { return i.test(this._text); };
    j.prototype.toStruct = function() { return { text: this._text, type: this._type, uid: this._uid, isLocal: this._isLocal, categoryName: this._categoryName }; };
    j.prototype.getHash = function(k) { var l = k != null ? this._getTypeParts().slice(0, k).join(':') : this._type; return String(l) + '::' + this._text; };
...
__d('FacebarStructuredText', ['FacebarStructuredFragment', 'createArrayFromMixed'], (function a(b, c, d, e, f, g) {
    ...
    function i(m) { if (!m) { return []; } else if (m instanceof l) { return m.toArray(); } else return c('createArrayFromMixed')(m).map(function(n) { return new(c('FacebarStructuredFragment'))(n); }); }

    function j(m) { return new(c('FacebarStructuredFragment'))({ text: m, type: 'text' }); }

    function k(m, n, o) {
        var p = m.getText(),
            q = p.replace(n, o);
        if (p != q) { return new(c('FacebarStructuredFragment'))({ text: q, type: m.getType(), uid: m.getUID() }); } else return m;
    }
    ...
    l.prototype.matches = function(m, n) {
        if (m) {
            var o = this._fragments,
                p = m._fragments;
            return o.length == p.length && !o.some(function(q, r) { var s = void 0; if (!n && q.getUID()) { s = q.getUID() != p[r].getUID(); } else s = q.getText() != p[r].getText() || q.getType() != p[r].getType(); return s || q.isLocal() != p[r].isLocal(); });
        }
        return false;
    };
    ...
    l.prototype.pad = function() { var m = this.getFragment(-1); if (m && !h.test(m.getText()) && m.getText() !== '') { return new l(this._fragments.concat(j(' '))); } else return this; };
    l.prototype.forEach = function(m) { this._fragments.forEach(m); return this; };
    l.prototype.matchType = function(m) {
        ...
        for (var o = 0; o < this._fragments.length; o++) {
            ...
            if (q && !n) { n = p; } else if (q || !p.isWhitespace()) return null;
        }
        return n;
    };
    l.prototype.hasType = function(m) { var n = arguments; return this._fragments.some(function(o) { return !o.isWhitespace() && o.isType.apply(o, n); }); };
    l.prototype.isLocal = function() { return this._fragments.some(function(m) { return m.isLocal(); }); };
    l.prototype.getCategoryName = function() { var m = this._fragments.filter(function(n) { return n.getCategoryName(); }); if (m.length > 0) return m[0].getCategoryName(); return null; };
    l.prototype.endsOnType = function(m) {
        var n = arguments,
            o = this._fragments[this._fragments.length - 1];
        return !!o && !o.isWhitespace() && o.isType.apply(o, n);
    };
    l.prototype.isEmptyOrWhitespace = function() { return !this._fragments.some(function(m) { return !m.isWhitespace(); }); };
    l.prototype.hasRTL = function() { return this._fragments.some(function(m) { return m.hasRTL(); }); };
    l.prototype.isEmpty = function() { return this.getLength() === 0; };
    l.prototype.getFragment = function(m) { return this._fragments[m >= 0 ? m : this._fragments.length + m]; };
    l.prototype.getCount = function() { return this._fragments.length; };
    l.prototype.getLength = function() { return this._fragments.reduce(function(m, n) { return m + n.getLength(); }, 0); };
    l.prototype.toStruct = function() { return this._fragments.map(function(m) { return m.toStruct(); }); };
    l.prototype.toArray = function() { return this._fragments.slice(); };
    l.prototype.toString = function() { return this._fragments.map(function(m) { return m.getText(); }).join(''); };
    l.prototype.getHash = function() { if (this._hash === null) this._hash = this._fragments.map(function(m) { if (m.getUID()) { return '[[' + m.getHash(1) + ']]'; } else return m.getText(); }).join(''); return this._hash; };
    l.fromStruct = function(m) { return new l(i(m)); };
    l.fromString = function(m) { return new l([j(m.toString())]); };
    f.exports = l;
}), null);
__d('QueryHistory', [], (function a(b, c, d, e, f, g) {
    var h = {},
        i = { set: function j(k, l) { h[this._key(k)] = l; }, get: function j(k) { return h[this._key(k)]; }, _key: function j(k) { return 'uri-' + k.getQualifiedURI().toString(); } };
    f.exports = i;
}), null);
__d('FacebarNavigation', ['csx', 'Arbiter', 'DOMQuery', 'FacebarBootloader', 'FacebarStructuredText', 'Input', 'QueryHistory', 'URI'], (function a(b, c, d, e, f, g, h) {
    ...
    c('Arbiter').subscribe('page_transition', function(r, s) { if (!q.isTopControlTransition(s.uri) && !q.isTimelineAbout(s.uri)) o(c('QueryHistory').get(s.uri), true); });
    c('Arbiter').subscribe('save_facebar_query', function(r, s) { n = true; });
    var q = {
        ...
        isTopControlTransition: function r(s) { return String(s.getPath()).startsWith('/search/') && s.getQueryData().ref === 'top_filter' && !s.getQueryData().hard_refresh; },
        isTimelineAbout: function r(s) { return /\/about$/.test(s.getPath()) && !s.getQueryData().hard_refresh; }
    };
    f.exports = q;
}), null);
    ...
    var h = null,
        ...
        j = {
            registerInput: function k(l, m) {
                ...
                c('Arbiter').subscribe('page_transition', function(n, o) { this._updateTitle(c('QueryHistory').get(o.uri)); }.bind(this));
            },
            ...
            setPageTitle: function k(l, m) {
                ...
                if (i) { this._updateTitle(l, m); } else h = l;
                var n = {};
                c('Arbiter').inform('search/updateNullState', n, c('Arbiter').BEHAVIOR_STATE);
            },
            setPageQuery: function k(l) { c('Arbiter').inform('search/updateNullState', l, c('Arbiter').BEHAVIOR_STATE); }
        };
    f.exports = j;
}), null);
__d("UIPageletContentCache", [], (function a(b, c, d, e, f, g) {
    var h = { cache: {}, getContent: function i(j) { if (j in this.cache) return this.cache[j]; return null; }, setContent: function i(j, k) { this.cache[j] = k; } };
    f.exports = h;
}), null);
__d('UIPagelet', ['ActorURI', 'AjaxPipeRequest', 'AsyncRequest', 'DOM', 'HTML', 'ScriptPathState', 'UIPageletContentCache', 'URI', 'emptyFunction', 'ge', 'isElementNode'], (function a(b, c, d, e, f, g) {
    ...
    h.prototype.getElement = function() { 'use strict'; return this._element; };
    ...
    h.prototype.refresh = function() {
        ...
        if (this._use_ajaxpipe) {
            ...
            if (this._cache_content) { var i = c('UIPageletContentCache').getContent(this._content_cache_key); if (i !== null) { this.handleContent(i); return this; } }
            var j = function(n) {
                    this._request = null;
        ...
        this._request.setURI(this._src).setAllowCrossPageTransition(this._allow_cross_page_transition).setData({ data: JSON.stringify(m) }).send();
        return this;
    };
    h.prototype.handleContent = function(i) {
        'use strict';
        if (this._append) { c('DOM').appendContent(this._element, i); } else c('DOM').setContent(this._element, i);
        this._handler();
    };
    h.prototype.cancel = function() { 'use strict'; if (this._request) this._request.abort(); };
    ...
    h.loadFromEndpointBatched = function(i, j, k) {
        ...
        if (m.length > 0) {
            ...
            n.options = babelHelpers['extends']({}, n.options, {
                    ...
                    o();
                    window.setTimeout(function() { h.loadFromEndpointBatched(m, j, k); }, 1);
                }
            });
        }
        l.forEach(function(p) { h.loadFromEndpoint(p.controller, p.target_element, p.data, babelHelpers['extends']({}, p.options, j, { bundle: true })); });
    };
    f.exports = h;
}), null);
__d('UITinyViewportAction', ['Arbiter', 'ArbiterMixin', 'BanzaiScuba', 'CSS', 'Event', 'getDocumentScrollElement', 'queryThenMutateDOM'], (function a(b, c, d, e, f, g) {
    var h = document.documentElement,
        ...
        q = {
            init: function r(s) {
                var t = c('queryThenMutateDOM').bind(null, function() {
                    ...
                    if (i !== m || j !== n || k !== o) {
                        ...
                        q.inform('change', i);
                        c('Arbiter').inform('tinyViewport/change', { tiny: i, tinyWidth: j, tinyHeight: k }, c('Arbiter').BEHAVIOR_STATE);
                        m = i;
                        n = j;
                        o = k;
                    }
                    if (!p) {
                        var u = new(c('BanzaiScuba'))('www_tinyview_port', null, { addBrowserFields: true });
            ...
            isTiny: function r() { return i; },
            isTinyWidth: function r() { return j; },
            isTinyHeight: function r() { return k; }
        };
...
__d('Button', ['csx', 'cx', 'CSS', 'DataStore', 'DOM', 'Event', 'Parent', 'emptyFunction', 'isNode'], (function a(b, c, d, e, f, g, h, i) {
    ...
    function r(w) { var x = c('Parent').byClass(w, 'uiButton') || c('Parent').bySelector(w, "._42ft"); if (!x) throw new Error('invalid use case'); return x; }

    function s(w) { return c('DOM').isNodeOfType(w, 'a'); }

    function t(w) { return c('DOM').isNodeOfType(w, 'button'); }

    function u(w) { return c('CSS').matchesSelector(w, "._42ft"); }
    var v = {
        getInputElement: function w(x) { x = r(x); if (s(x)) throw new Error('invalid use case'); return t(x) ? x : c('DOM').find(x, 'input'); },
        isEnabled: function w(x) { return !(c('CSS').hasClass(r(x), j) || c('CSS').hasClass(r(x), l)); },
        ...
        isDepressed: function w(x) { x = r(x); var y = u(x) ? m : k; return c('CSS').hasClass(x, y); },
        setLabel: function w(x, y) {
            ...
            if (u(x)) {
                ...
                if (aa)
                    if (x.firstChild == aa) { z.unshift(aa); } else z.push(aa);
        ...
        getIcon: function w(x) { x = r(x); return c('DOM').scry(x, '.img')[0]; },
        setIcon: function w(x, y) {
            if (y && !c('isNode')(y)) return;
            var z = v.getIcon(x);
            if (!y) { z && c('DOM').remove(z); return; }
            c('CSS').addClass(y, 'customimg');
            if (z != y)
                if (z) { c('DOM').replace(z, y); } else c('DOM').prependContent(r(x), y);
        }
    };
    f.exports = v;
}), null);
__d('BehaviorsMixin', [], (function a(b, c, d, e, f, g) {
    ...
    function j(l) { if (!l.__BEHAVIOR_ID) l.__BEHAVIOR_ID = i++; return l.__BEHAVIOR_ID; }
    var k = {
        ...
        disableBehavior: function l(m) { if (this._behaviors) { var n = j(m); if (this._behaviors[n]) this._behaviors[n].disable(); } return this; },
        enableBehaviors: function l(m) { m.forEach(this.enableBehavior, this); return this; },
        ...
        hasBehavior: function l(m) { return this._behaviors && j(m) in this._behaviors; }
    };
    f.exports = k;
}), null);
__d('UIForm', ['ArbiterMixin', 'BehaviorsMixin', 'DOM', 'Event', 'Form', 'Run', 'areJSONRepresentationsEqual', 'mixin'], (function a(b, c, d, e, f, g) {
    ...
    function j(k, l, m, n, o) {
        ...
        if (n) {
            ...
            c('DOM').prependContent(this._root, c('DOM').create('input', { type: 'hidden', name: 'confirmed', value: 'true' }));
        }
        c('Run').onAfterLoad(function() { this._originalState = c('Form').serialize(this._root); }.bind(this));
        this._forceDirty = m;
    ...
    j.prototype.getRoot = function() { 'use strict'; return this._root; };
    j.prototype._handleSubmit = function() {
        'use strict';
        if (this._confirm_dialog && !this._confirmed) { this._confirm_dialog.show(); return false; }
    ...
    j.prototype._handleDialogConfirm = function() {
        ...
        if (this._root.getAttribute('ajaxify')) { c('Event').fire(this._root, 'submit'); } else if (this._handleSubmit()) this._root.submit();
    };
    ...
    j.prototype.isDirty = function() { 'use strict'; if (this._submitted || !c('DOM').contains(document.body, this._root)) return false; if (this._forceDirty) return true; if (!this._originalState) return false; var k = c('Form').serialize(this._root); return !c('areJSONRepresentationsEqual')(k, this._originalState); };
    j.prototype.checkUnsaved = function() { 'use strict'; if (this.isDirty()) return this._message; return null; };
...
__d('SimpleFBAuthenticatedXHRRequest', ['invariant', 'CurrentUser', 'DTSG', 'ServerJSDefine', 'XHRRequest', 'isFacebookURI'], (function a(b, c, d, e, f, g, h) {
    ...
    function l(m, n) {
        ...
        var o = { __dyn: c('ServerJSDefine').getLoadedModuleHash(), __req: (k++).toString(36), __ajax__: 1, __a: 1, __user: c('CurrentUser').getID() };
        j.setData.call(this, babelHelpers['extends']({}, n, o));
    }
    l.prototype.send = function() { 'use strict'; if (!c('isFacebookURI')(this.getURI())) return j.send.call(this); if (c('DTSG').getCachedToken) { var m = c('DTSG').getCachedToken(); if (m) { return this.sendOnDTSGToken(m); } else { c('DTSG').getToken().done(function(n) { return this.sendOnDTSGToken(n); }.bind(this)); return this; } } else this.sendOnDTSGToken(c('DTSG').getToken()); };
    l.prototype.sendOnDTSGToken = function(m) { 'use strict'; if (m) j.setData.call(this, babelHelpers['extends']({}, this.getData(), { fb_dtsg: m })); return j.send.call(this); };
    l.prototype.setData = function(m) {
        'use strict';
        h(0);
    };
    l.parseResponse = function(m) { 'use strict'; return JSON.parse(m.substr(9)); };
    l.getPayload = function(m) { 'use strict'; var n = l.parseResponse(m).payload; return n.payload ? n.payload : n; };
    f.exports = l;
}), null);
...
__d('FantailLogQueue', ['ChannelClientID', 'CircularBuffer', 'FantailConfig', 'PHPQuerySerializer', 'SimpleFBAuthenticatedXHRRequest', 'XFantailLogController', 'destroyOnUnload', 'setIntervalAcrossTransitions', 'sprintf'], (function a(b, c, d, e, f, g) {
    var h = { DEBUG: 'debug', INFO: 'info', WARN: 'warn', ERROR: 'error' };
    ...
    i.prototype.$FantailLogQueue5 = function(j, k) {
        ...
        this.$FantailLogQueue3.write({ log_time: Date.now(), log: o, severity: j, device_id: c('ChannelClientID').getID() });
    };
    i.prototype.$FantailLogQueue4 = function() {
        'use strict';
        var j = this.$FantailLogQueue3.read();
        if (j.length > 0) {
            var k = { log_time: [], log: [], severity: [], device_id: [] };
...
__d('performanceNavigationStart', ['performance'], (function a(b, c, d, e, f, g) {
    ...
    if (c('performance').now) {
        if (c('performance').timing && c('performance').timing.navigationStart) { i = function j() { return c('performance').timing.navigationStart; }; } else if (typeof window._cstart === 'number') { i = function j() { return window._cstart; }; } else(function() {
            var j = Date.now();
            i = function k() { return j; };
        })();
    } else i = function j() { return 0; };
    f.exports = i;
}), null);